...
// (AiSettings.firstBandDelaySeconds, 5-60s); as outras duas continuam
...
// aqui (5-60) porque quem grava o valor é uma action de formulário, não o
...
export function computeAdaptiveDelaySeconds(
  ...
  if (leadResponseTimeSeconds === null || leadResponseTimeSeconds <= ONE_HOUR_SECONDS) {
    return Math.min(60, Math.max(5, Math.round(firstBandDelaySeconds)));
  }
